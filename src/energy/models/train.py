...
def load_reference(choice: str)->tuple[str, float]|None:
    """
    Nome e MAE della baseline di riferimento, se la baseline e' gia' stata eseguita.
    """
...
def main()->None:
    ...
    if reference is not None:
        ...
        print(f"Riferimento {reference_name}: MAE {reference_mae:.4f}\n")
    ...
    for name in names:
        ...
        if reference is not None:
            print(
                f"    Miglioramento su {reference_name}: "
                f"{payload['improvement_vs_reference']:.1%}\n"
            )
